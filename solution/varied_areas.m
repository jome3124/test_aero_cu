...
r_throat_avg = 0.123/2; % [in] throat radius
r_exit_avg = 0.231/2; % [in] exit radius
Mass = 0.030; % [kg]


%create a for loop to vary the throat and exit radii
for k = -10:10
    r_throat = r_throat_avg + k/2000;
    for i = k+1:10
        r_throat = r_throat_avg + k/2000;
        r_exit = r_exit_avg + i/1000;
        h_grain = 1.915; % [in] motor grain height
        r_grain = 0.87/2; % [in] motor grain radius
        %% CONVERSION
        ...
        if ~isempty(T_predicted)
            maxF(k+101,i+101) = max(T_predicted);
            maxP(k+101,i+101) = max(Pc);
        else
            maxF(k+101,i+101) = 0;
            maxP(k+101,i+101) = 0;
        end
